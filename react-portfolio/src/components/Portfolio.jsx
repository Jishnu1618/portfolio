...
import { FileText, Download, Eye, Briefcase, Calendar, MapPin } from 'lucide-react';
...
const matchesFilter = (item, activeFilter) => {
  ...
  return item.tags.some((tag) => {
    ...
    return tagLower.includes(needle) || needle.includes(tagLower);
  });
};

const Portfolio = ({ activeFilter, setActiveFilter }) => {
  const experience = portfolioData.filter((item) => item.type === 'Experience');
  const projects = portfolioData.filter((item) => item.type === 'Project');

  const renderCard = (item, index) => {
    const isFilteredOut = !matchesFilter(item, activeFilter);
    ...
    return (
      ...
      >
        <div className="absolute left-[20px] top-8 w-2 h-2 rounded-full bg-violet-400 shadow-[0_0_10px_rgba(139,92,246,0.8)] hidden sm:block" />
        ...
        <SpotlightCard className="p-6 md:p-8">
          ...
          <ul className="mt-6 space-y-3 text-zinc-300 list-disc list-inside marker:text-cyan-500">
            {item.description.map((desc, i) => (
              <li key={i} className="pl-2 leading-relaxed whitespace-normal text-sm md:text-base">
                {desc}
              </li>
            ))}
          </ul>
    ...
    >
      <motion.div variants={fadeInUp} className="mb-12">
        ...
        <p className="mt-3 text-zinc-400 max-w-2xl text-lg">
          Production internships and engineering projects spanning cloud DevOps, generative AI
          systems, edge ML, and full-stack data pipelines.
        </p>
        {activeFilter && (
          <div className="mt-4 flex flex-wrap gap-2 text-sm text-zinc-500 items-center">
            Filtered view for work involving{' '}
            <span className="badge-neon !text-fuchsia-300">{activeFilter}</span>
          </div>
        )}
      </motion.div>
      ...
      <motion.div variants={fadeInUp} className="mb-12">
        <SpotlightCard className="p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-6 border-cyan-500/20 bg-cyan-950/10">
          <div className="flex items-center gap-5 w-full md:w-auto">
            ...
            <div>
              <h4 className="font-bold text-xl md:text-2xl text-zinc-100 tracking-tight">
                Detailed Resume
              </h4>
              <p className="text-zinc-400 text-sm mt-1">
                Full professional history — systems, DevOps, AI, and embedded engineering.
              </p>
            </div>
          </div>
          <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
            <a href="/Resume_detailed.pdf" download className="btn-glass w-full sm:w-auto">
              <Download className="w-4 h-4 text-cyan-400" /> Download PDF
            </a>
            <a
              href="/Resume_detailed.pdf"
              target="_blank"
              rel="noreferrer"
              className="btn-glass bg-transparent border-zinc-700 w-full sm:w-auto"
            >
      ...
      <div className="space-y-16">
        <div>
          <motion.h4 variants={fadeInUp} className="text-xl font-semibold text-zinc-200 mb-8 pl-0 sm:pl-16">
            Professional Experience
          </motion.h4>
          <div className="space-y-8 relative">
            <div className="absolute left-6 top-0 bottom-0 w-px bg-gradient-to-b from-cyan-500/0 via-violet-500/20 to-fuchsia-500/0 hidden sm:block" />
        ...
        <div>
          ...
          <div className="space-y-8 relative">
            <div className="absolute left-6 top-0 bottom-0 w-px bg-gradient-to-b from-cyan-500/0 via-fuchsia-500/20 to-cyan-500/0 hidden sm:block" />
