...
const skillCategories = [
  ...
  { key: 'backend', label: 'Backend & Data Engineering' },
  { key: 'aiSystems', label: 'AI & Systems' },
  { key: 'cloudDevOps', label: 'Cloud & DevOps' },
  { key: 'methodologies', label: 'Domain & Methodologies' },
];

const SkillsDashboard = ({ activeFilter, setActiveFilter }) => {
  const allSkillsForChart = skillCategories
    .flatMap(({ key }) => skillsData[key])
    ...
    datasets: [
      {
        ...
        hoverBorderColor: 'rgba(217, 70, 239, 1)',
      },
    ],
  };
  ...
  return (
    ...
    >
      <motion.div variants={fadeInUp} className="mb-12">
        <h3 className="text-3xl md:text-4xl font-bold text-zinc-100 tracking-tight">Technical Skills</h3>
        <p className="mt-3 text-zinc-400 max-w-2xl text-lg">
          Full-stack systems engineering across backend services, generative AI orchestration, cloud
          infrastructure, and embedded edge computing. Click any skill to filter related work below.
        </p>
        {activeFilter && (
          <div className="mt-4 flex items-center gap-3">
            ...
            <button
              ...
              className="text-sm text-zinc-500 hover:text-cyan-400 underline underline-offset-4"
            >
              Reset
            </button>
          </div>
      ...
      <motion.div variants={fadeInUp} className="spotlight-card p-4 sm:p-2 mb-8">
        <div className="chart-container relative w-full h-[500px] md:h-[650px]">
      ...
      <motion.div
        variants={fadeInUp}
        className="spotlight-card p-6 md:p-8 flex flex-col md:flex-row flex-wrap gap-10"
      >
        {skillCategories.map(({ key, label }) => renderBadgeList(skillsData[key], label))}
      </motion.div>
    </motion.section>
  );
};
