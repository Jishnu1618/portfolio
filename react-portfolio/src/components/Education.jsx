...
const GalleryLauncher = () => {
    const [open, setOpen] = useState(false);
    const [images, setImages] = useState([]);

    // Try to load the drive-photos.json generated by the script
    const loadImages = async () => {
        try {
            const res = await fetch('/drive-photos.json');
            if (!res.ok) throw new Error('no json');
            const data = await res.json();
            setImages(data.slice(0, 16));
        } catch (e) {
            // fallback to placeholders
            const placeholders = Array.from({ length: 16 }).map((_, i) => `https://images.unsplash.com/photo-1504198266280-5f3b1d1d9b3a?auto=format&fit=crop&w=1000&q=80&sig=${i}`);
    ...
    return (
        <>
            <div className="mt-6 inline-block">
                <button onClick={() => { loadImages(); setOpen(true); }} className="btn-glass border-zinc-700 bg-transparent text-zinc-300 inline-flex items-center gap-2">
                    <Image className="w-4 h-4 text-fuchsia-400" /> View Photography Collection
                </button>
            </div>
            <PhotoGallery images={images} open={open} onClose={() => setOpen(false)} />
        </>
    );
};
